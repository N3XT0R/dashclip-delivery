...
- **Look at the user area as someone else**: Administrators can open the view of a user from the user list and see
  ...
  it again. Downloads are not recorded meanwhile, so the channel keeps its own history, and the start and the end of
  every such view are written to the activity log. Administrators cannot be opened this way.
