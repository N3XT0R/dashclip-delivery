...
<article class="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-panel shadow-sm">
    ...
    <div class="flex flex-1 flex-col items-start gap-4 {{ $compact ? 'p-4' : 'p-5' }}">
        <x-public.blog.category-badge :category="$article->post->category" />
        <h2 class="{{ $compact ? 'text-base' : 'text-xl' }} font-bold leading-snug"><a href="{{ $blog->url('show', $article->locale, ['slug' => $article->slug]) }}" class="hover:underline">{{ $article->title }}</a></h2>
        <p class="{{ $compact ? 'text-xs' : 'text-sm' }} text-muted">{{ $article->excerpt }}</p>
