...
<footer class="border-t border-border bg-panel">
    <div class="public-width py-14">
        <h2 class="mb-6 text-sm font-semibold tracking-widest uppercase">Kanäle</h2>
        ...
        <div class="mt-12 flex flex-col justify-between gap-8 border-t border-border pt-8 lg:flex-row">
            <div>
                ...
                <p class="mt-2 text-sm text-muted">Ein Upload. Passende Kanäle. Nachvollziehbare Angebote.</p>
                <p class="mt-3 text-xs text-muted">&copy; {{ date('Y') }} · Version <a class="underline" href="{{ route('changelog') }}">{{ Version::getCurrentVersion() }}</a></p>
            </div>
            <nav aria-label="Rechtliches und Projekt" class="flex max-w-xl flex-wrap content-start gap-x-5 gap-y-3 text-sm text-muted">
                @foreach (['impressum' => 'Impressum', 'datenschutz' => 'Datenschutz', 'tos' => 'Nutzungsbedingungen', 'license' => 'Lizenz', 'api-docs' => 'API-Docs', 'changelog' => 'Changelog'] as $name => $label)
                    <a class="underline-offset-4 hover:underline" href="{{ route($name) }}">{{ $label }}</a>
                @endforeach
                <a href="{{ url(config('app.footer.roadmap')) }}">Roadmap</a>
                <a href="{{ url(config('app.footer.issues')) }}">Bug gefunden?</a>
                <a href="https://github.com/N3XT0R/dashclip-delivery">GitHub</a>
            </nav>
        </div>
        @unless (app()->environment('production'))
            <p class="mt-8 rounded-lg border border-amber-600 bg-amber-50 p-3 text-sm font-semibold text-amber-950">Testinstanz - Nicht zur produktiven Verwendung</p>
        @endunless
    </div>
</footer>
