...
@section('title', __('tokens.channel_access.title'))
@section('subtitle', $channel?->name ?? __('tokens.channel_access.subtitle'))
...
@section('content')
    <div class="panel" style="max-width: 600px; margin: 0 auto; text-align: center; padding: 32px;">
        <h1 style="font-size: 22px; margin-bottom: 16px; color: var(--color-primary, #2563eb);">
            {{ __('tokens.channel_access.headline') }}
        </h1>

        <p style="margin-bottom: 16px;">
            {{ __('tokens.channel_access.thanks') }}
        </p>

        <p style="line-height: 1.6;">
            {{ __('tokens.channel_access.description', [
                'channel' => $channel?->name ?? __('tokens.channel_access.subtitle'),
            ]) }}
        </p>

        <p style="margin-top: 20px;">
            {{ __('tokens.channel_access.access_granted') }}
        </p>

        <p style="margin-top: 12px;">
            {{ __('tokens.channel_access.revoke_notice') }}
        </p>

        <div style="margin-top: 24px;">
            <a href="{{ config('app.url') }}" class="btn" style="text-decoration: none;">
                {{ __('tokens.channel_access.back') }}
            </a>
        </div>
